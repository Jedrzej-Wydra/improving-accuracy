...
K_adjust_10_OPT_2 <- MAPE(model_adjust_10_OPT$KT,true_model$KT)
K_r_10_OPT_2 <- MAPE(mr10_OPT,true_model$KT)

comparison_R_4 <- data.frame(Model=c("K_adjust_10_OPT",
                                     "K_r_10_OPT",
                                     "K_general",
                                     "K_random",
                                     "K_OLS",
                                     "K_OLS_REV"),
                             MSE=c(K_adjust_10_OPT_2,
                                   K_r_10_OPT_2,
                                   K_general_2,
                                   K_random_2,
                                   K_OLS_2,
                                   K_OLS_REV_2),
                             colour=c("J","R","G","R","O","V"))
ggplot(comparison_R_4,aes(x=reorder(Model,MSE),y=MSE,fill=colour))+geom_bar(stat="identity",show.legend = FALSE)+theme_minimal()+xlab("Model")+ylab("MAPE")
